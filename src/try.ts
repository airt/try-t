...
export default abstract class Try<A> implements Iterable<A> {
  ...
  static p = <A>(p: Promise<A>): Promise<Try<A>> =>
    p.then(Try.success).catch(e => Try.failure(e))
...
class Failure<A> extends Try<A> {
  ...
  recover = (f: Fn<Error, A>): Try<A> => Try.of(() => f(this.error))

  recoverWith = (f: Fn<Error, Try<A>>): Try<A> => Try.m(() => f(this.error))
  ...
  filter = (p: (x: A) => boolean): Try<A> =>
    this.flatMap(
      x =>
        p(x)
          ? this
          : Try.failure(new Error(`predicate does not hold for ${this.value}`)),
    )

  flatMap = <B>(f: Fn<A, Try<B>>): Try<B> => Try.m(() => f(this.value))
...
export { Failure as _Failure, Success as _Success }
